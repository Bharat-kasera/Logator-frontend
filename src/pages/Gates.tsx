...
const Gates: React.FC = () => {
  ...
  const maxGates = plan === 1 ? 2 : plan === 2 ? 10 : Infinity; // Basic: 2 gates, Pro: 10 gates, Enterprise: unlimited
  ...
  const getPlanDescription = () => {
    if (isBasic) return 'Basic plan allows up to 2 gates';
    if (isPro) return 'Pro plan allows up to 10 gates';
    if (isEnterprise) return 'Enterprise plan allows unlimited gates';
          ...
          <div className="space-y-6">
            ...
            {!editingGate && gates.length >= maxGates && (
              <p className="text-sm text-gray-500">
                You have reached the maximum number of gates for your plan.
              </p>
            )}
          </div>
        </div>
